...
export default class ProductContoller {
  ...
  async insertProduct(request: Request, response: Response, next: NextFunction) {
  ...
  async editProduct(request: Request, response: Response, next: NextFunction) {
    try { 
      ...
      const editedProduct = await this.productService.editProduct(productId, productToEdit)
